...
class MainApp(App):
    ...
    def carregar_infos_usuario(self):
        try:
            ...
            requisicao = requests.get(
                f"https://aplicativovendashash-7a4b8-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}")
            ...
            try:
                ...
                for id_venda  in vendas:
                    ...
                    banner = BannerVenda(cliente=venda["cliente"], foto_cliente=venda["foto_cliente"],
                                         ...
                                         data=venda["data"], precp=venda["preco"], unidade=venda["unidade"],
    ...
    def mudar_foto_perfil(self, foto, *args):
        ...
        info = f'{{"avatar": "{foto}"}}'
        requisicao = requests.patch(f"https://aplicativovendashash-7a4b8-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                                    data=info)
        self.mudar_tela("ajustespage")


    def adicionar_vendedor(self, id_vendedor_adicionado):
        link = f'https://aplicativovendashash-7a4b8-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}"'
        ...
        if requisicao_dic == {}:
            ...
            if id_vendedor_adicionado in equipe:
                ...
                requests.patch(f"https://aplicativovendashash-7a4b8-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}", data=info)
    ...
    def selecionar_unidade(self, id_label, *args):
        ...
        # pintar o cara selecionao de azul
    ...
    def adicionar_venda(self):
        ...
        if cliente and produto and unidade and preco and quantidade and (type(preco) == float) and type(quantidade) == float:
            ...
            requests.post(f"https://aplicativovendashash-7a4b8-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}", data=info)
            ...
            requisicao = requests.get(f"https://aplicativovendashash-7a4b8-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}")
    ...
    def carregar_todas_vendas(self):
        ...
        requisicao = requests.get(
            f'https://aplicativovendashash-7a4b8-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"')
        ...
        if requisicao.status_code == 200:
            ...
            for local_id_usuario, dados_usuario in requisicao_dic.items():
                if isinstance(dados_usuario, dict):
                    vendas = dados_usuario.get('vendas', {})

                    if isinstance(vendas, dict):
                        ...
                        print(f"Usuário {local_id_usuario} não tem vendas ou o campo 'vendas' não é um dicionário.")
                else:
                    print(f"Os dados do usuário {local_id_usuario} não são um dicionário.")
    ...
    def carregar_vendas_vendedor(self, dic_info_vendedor, *args):
        ...
        if isinstance(dic_info_vendedor, dict):
            vendas = dic_info_vendedor.get('vendas', {})
            ...
            if isinstance(vendas, dict):
                ...
                print(f"Usuário não tem vendas ou o campo 'vendas' não é um dicionário.")
        else:
            print(f"Os dados do usuário não são um dicionário.")

        # preencher total de vendas
        total_vendas = dic_info_vendedor.get('total_vendas', {})
